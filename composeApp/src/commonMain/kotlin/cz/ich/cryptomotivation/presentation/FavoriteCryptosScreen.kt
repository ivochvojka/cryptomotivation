...
/**
 * Screen with favorite cryptocurrencies.
 * Cryptocurrencies must be added to be shown here.
 */
class FavoriteCryptosScreen : Screen {
    @Composable
    ...
    private fun CryptosContent(cryptoList: List<CryptoData>) {
        ...
        Scaffold(
            topBar = {
                AppTopAppBar(stringResource(Res.string.favorites_title))
            },
            bottomBar = {
                CryptoNavigationBar(
                    selectedDestination = CryptocurrencyBarItem.Favorite,
                    onDestinationSelected = {
                        navigator.replace(CryptosScreen())
                    },
                )
            }
        ) { paddingValues ->
